...
select a.cust_no uid,nvl(eduscore,0)+nvl(pfscore,0)+nvl(tagescore,0)+nvl(hfscore,0)+nvl(rds,0)+nvl(nfs,0)
        +nvl(assets,0)+nvl(bnks,0)+nvl(ps,0)+nvl(rs,0)+nvl(fs,0)+nvl(hs,0)+nvl(risks,0)+nvl(amts,0)+nvl(vts,0) score
	 ,eduscore,pfscore,tagescore,hfscore,nfs,rds,assets,bnks,ps,rs,fs,hs,risks,amts,vts
from (
...
case when sumamt>=500000 then 2.7
    when sumamt>=100000 then 2.2
    when sumamt>=2000 then 1.8
    else 1.2
    end amts
    from
...
left join
(
 select distinct cust_no, 1.8 rds
from dccust.tb_fact_cust_ofs_trd_detail
...
select trim(cust_no) cust_no,count(event_time)vtimes, --distinct cust_no
    case when count(event_time)>=10 then 2.6
        when count(event_time)>=5 then 2.4
        when count(event_time)>=2 then 2.2
        when count(event_time)=1 then 1.5
        end vts
